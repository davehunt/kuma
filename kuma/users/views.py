...
def user_detail(request, username):
    ...
    if (detail_user.is_banned and not request.user.is_superuser):
...
def user_edit(request, username):
    ...
    if not user.allows_editing_by(request.user):
    ...
    already_subscribed = NewsletterForm.is_subscribed(user.email)

    if request.method != 'POST':
        initial = {
            'beta': user.is_beta_tester,
            'username': user.username,
        }
        # Load up initial websites with either user data or required base URL
        for name, meta in User.WEBSITE_CHOICES:
            initial['websites_%s' % name] = user.websites.get(name, '')

        # Form fields to receive tags filtered by namespace.
        for field, ns in field_to_tag_ns:
            initial[field] = ', '.join(t.name.replace(ns, '')
                                       for t in user.tags.all_ns(ns))
        ...
        user_form = UserEditForm(instance=user, initial=initial, prefix='user')
        ...
        # posted_username = request.POST.get('user-username', None)
        ...
        if user_form.is_valid() and newsletter_form.is_valid():
            ...
            for name, meta in User.WEBSITE_CHOICES:
                ...
                field_value = user_form.cleaned_data.get(field_name, '')
                if field_value and field_value != meta['prefix']:
                    sites[name] = field_value
            new_user.websites = sites

            # Save the user now, since the rest of this deals with
            # related resources...
            new_user.save()
